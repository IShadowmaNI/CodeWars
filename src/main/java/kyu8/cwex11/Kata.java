...
public class Kata {
    ...
    public static int seatsInTheater(int nCols, int nRows, int col, int row) {
        ...
        if (nCols > 1000 || nRows > 1000 || col > 1000 || row > 1000) {
            throw new IllegalArgumentException("One or more arguments exceeded the maximum limit!");
        } else if (nCols < 1 || nRows < 1 || col < 1 || row < 1) {
            throw new IllegalArgumentException("One or more arguments is less than the minimum limit!");
        } else if (col > nCols || row > nRows) {
            throw new IllegalArgumentException("Variables col & row should be less than nCols & nRows!");
        }
